...
export default function PayslipPage() {
  ...
  const openPreview = () => {
    setPreviewMarkup(capturePrintableMarkup());
    setPreviewOpen(true);
  };
  ...
  const handleSaveRecord = async () => {
    ...
    try {
      ...
      if (data.success && data.payslip) {
        ...
        setTimeout(() => setShowPostSaveActions(false), 6000);
      }
    } catch (err) {
    ...
    <div className="space-y-5">
      ...
      <section className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center no-print border-b border-white/10 pb-4">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          >
            {saving ? "Generating..." : saveSuccess ? "Generated" : "Generate Payslip"}
          </button>
      ...
      {showPostSaveActions && (
        <div className="no-print flex flex-wrap items-center gap-2 rounded-2xl border border-white/10 bg-white/5 px-3 py-2 text-xs text-white/70">
          <span className="font-medium text-white/85">Payslip generated successfully.</span>
          <button
            type="button"
            onClick={handleDownloadPayslip}
            className="rounded-xl border border-white/10 bg-white/5 px-3 py-1.5 font-medium text-white/80 transition hover:bg-white/10 hover:text-white"
          >
            Download PDF
          </button>
          <button
            ...
            className="rounded-xl bg-gradient-to-r from-accent to-accent-strong px-3 py-1.5 font-semibold text-white shadow-md shadow-accent/20 transition hover:brightness-110"
          >
            Print
          </button>
        </div>
      ...
      {previewOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/90 p-2 sm:p-4 backdrop-blur-sm no-print">
          <div className="relative flex h-[86vh] w-full max-w-5xl flex-col overflow-hidden rounded-3xl border border-white/10 bg-surface shadow-2xl">
            <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
              <div>
                <p className="text-[10px] uppercase tracking-[0.2em] text-white/45">Full Preview</p>
                <h3 className="text-base font-semibold text-white">Payslip Preview</h3>
              </div>
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={handleDownloadPayslip}
                  className="rounded-xl border border-white/10 bg-white/5 px-2.5 py-1.5 text-[11px] font-medium text-white/80 hover:bg-white/10 hover:text-white transition"
                >
                  Download PDF
                </button>
                <button
                  type="button"
                  onClick={handlePrintPayslip}
                  className="rounded-xl bg-gradient-to-r from-accent to-accent-strong px-2.5 py-1.5 text-[11px] font-semibold text-white shadow-md shadow-accent/20 hover:brightness-110 transition"
                >
                  Print
                </button>
                <button
                  ...
                  className="rounded-xl border border-white/10 bg-white/5 p-1.5 text-white/70 hover:bg-white/10 hover:text-white transition"
                >
            ...
            <div className="flex-1 overflow-auto bg-slate-200 p-2 sm:p-3">
